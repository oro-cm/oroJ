...
public class OroSystem {
    private static final Logger log = LoggerFactory.getLogger(OroSystem.class);
    ...
    OroSystem(String apiUrl, AbstractBlockChain blockChain, NetworkParameters networkParameters) throws OroSystemException {
        try {
            this.grsApi = new GrsApi(apiUrl);
        } catch (MalformedURLException e) {
            throw new OroSystemException("Malformed live feed URL in GRS API", e);
        }
    ...
    public boolean checkBlockReward(Block block, int height, Coin coinbaseValue, Coin fees) throws BlockStoreException {
        log.info("OroSystem::checkBlockReward() : block.time = {}, fees = {}, hash = {}, height = {}", block.getTimeSeconds(), fees.toString(), block.getHashAsString(), height);

        BlockStore blockStore = blockChain.getBlockStore();

        if (coinbaseValue.isZero() || coinbaseValue.compareTo(fees) < 0) {
            throw new VerificationException("OroSystem::checkBlockReward() : coinbase pays zero or <= fees (coinbase="
        ...
        if (block.getTimeSeconds() > networkParameters.getLiveFeedSwitchTime()) {
            ...
            throw new VerificationException("OroSystem::checkBlockReward() : coinbase pays wrong (actual="
        ...
        if (!blockReward.equals(getExpectedBlockReward(block, height))) {
            throw new VerificationException("OroSystem::checkBlockReward() : coinbase pays wrong reward (actual="
    ...
    public Coin getExpectedBlockReward(Block block, int height) throws BlockStoreException {
        log.info("OroSystem::getExpectedBlockReward(): hash = {}, height = {}, time = {}", block.getHashAsString(), height, block.getTimeSeconds());
...
//        Coin reward = 0; // TODO(oro): get block for time, and then its reward
...
//                    boost::this_thread::interruption_point();
                    long timestamp = 0; //TODO(oro): must be 'time'
...
//                }
        ...
        public Usd getLatestPrice() {
            return Usd.valueOf(0, 10);   // STUB: 0.1USD, TODO(oro): get actual coin price
        }
